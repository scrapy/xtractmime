...
class TestMain:
    ...
    @pytest.mark.parametrize(
        "supplied_type,http_origin,expected",
        [
    ...
    def test_should_check_for_apache_bug(self, supplied_type, http_origin, expected):
        assert (
            _should_check_for_apache_bug(supplied_type=supplied_type, http_origin=http_origin)
            == expected
        )

    @pytest.mark.parametrize(
        "input_bytes,byte_pattern,pattern_mask,whitespace,expected",
        [
    ...
    def test_is_match_mime_pattern(
        self, input_bytes, byte_pattern, pattern_mask, whitespace, expected
    ):
